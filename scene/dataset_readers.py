...
import cv2
from utils.s3_general_utils import sample_on_aabb_surface, get_OccGrid
        ...
        for t in trange(0, len(lidar_filepaths), desc="loading lidar", dynamic_ncols=True):
            ...
            if load_depthmap:
                ...
                for cam_idx in range(len(camera_list)):
                    ...
                    depth_map[image_points[:, 1].astype(np.int32), image_points[:, 0].astype(np.int32)] = pixel_points[
                                                                                                          :, 2]
...
def GridSample3D(in_pc, in_shs, voxel_size=0.013):
    ...
    voxel_index = quantized_pc[:, 0] * pc_boundary[1] * pc_boundary[2] + quantized_pc[:, 1] * pc_boundary[
        2] + quantized_pc[:, 2]
